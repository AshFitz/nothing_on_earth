...
def all_products(request):
    ...
    products = (
        Product.objects.filter().order_by("collection").exclude(sale=True)
    )
    query = None
    collections = None
    sort = None
...
def add_review(request, product_id):
    ...
    if request.method == "POST":
        ...
        form_data = {"comment": request.POST["comment"]}
        ...
        if review_form.is_valid():
            ...
            review.save()
            messages.info(
                request, f"Thank you for posting a review for {product.name}."
            )
        else:
            messages.error(
                request,
                f"Sorry we were unable to post a review for {product.name}, try again.",
            )
...
def edit_review(request, product_id):
    ...
    if request.method == "POST":
        ...
        if review_form.is_valid():
            ...
            messages.error(
                request,
                f"Sorry an error has accoured, we cannot update your review for {product.name}, please try again.",
            )
    else:
        review_form = ReviewForm(instance=user)
